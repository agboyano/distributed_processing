...
class FSUDict:
    ...
    def keys(self) -> list[str]:
        return [self._filename_to_key(f.name) for f in os.scandir(self.base_path) if f.is_file()]
...
class FSList():
    ...
    def __init__(self, base_path: str, temp_dir: str, serializer=joblib_serializer, ext="pkl", prec=50):
        ...
        self._zero_date = datetime.datetime.combine(datetime.date(2025, 1, 1), datetime.time(0, 0, 0))
        self._zero_timestamp = Decimal(math.ceil(datetime.datetime.timestamp(self._zero_date)*1000000))/1000000
    ...
    def _seconds_elapsed(self):
        return Decimal(math.ceil(datetime.datetime.timestamp(datetime.datetime.now())*1000000))/1000000 - self._zero_timestamp
    ...
    def _new_mid_key(self, current_key, current_prev_key):
        with localcontext() as ctx:
            ...
            if new == prev:
                raise IndexError(f"Insert: Key collision: Increment precision above {self.prec}")
    ...
    def keys(self):
        with localcontext() as ctx:
            ctx.prec = self.prec
            return [k for _, k in sorted([(Decimal(k[0]), k) for k in self.data.keys()], key=lambda x:x[0])]    
...
class FSNamespace:
    ...
    def udict(self, name):
        ...
        return FSUDict(self.base_path / (self.udict_prefix + self.sep + name), self.temp_dir, self.serializer)
    
    def list(self, name):
        nt = [x for x in self.names_types() if x[0]==name]
        if len(nt)>0 and nt[0][1]!=self.list_prefix:
            raise ValueError(f"{name} exits with type {nt[0][1]}")  
        return FSList(self.base_path / (self.list_prefix + self.sep + name), self.temp_dir, self.serializer)
    ...
    def names_types(self):
        tmp = [self._dirname_to_name_type(d.name) for d in self.base_path.iterdir() if d.is_dir()]
